...
This downloads the `allure-evidence` artifact into `tests/artifacts/ci-allure` and serves
the generated static report locally. Use `RUN_ID=<github-run-id>` to inspect a specific
workflow run, or `PORT=5253` if port `5252` is already in use.
